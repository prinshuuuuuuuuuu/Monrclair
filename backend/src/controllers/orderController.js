...
const createOrder = async (req, res) => {
  ...
  try {
    ...
    const [orderResult] = await connection.query(
      'INSERT INTO orders (user_id, total_amount, shipping_address, status) VALUES (?, ?, ?, "processing")',
    ...
    // MOCK NOTIFICATION
    console.log(`[NOTIFICATION] Order confirmed for user ${req.user.id}: Order ID ${orderId}`);
...
const cancelOrder = async (req, res) => {
  ...
  try {
    ...
    if (orders[0].status !== 'processing') return res.status(400).json({ message: 'Cannot cancel order in current state' });
